                                ...
                                <img alt="<?php echo esc_attr(__('Image', 'majestic-support')); ?>" src="<?php echo esc_url(MJTC_PLUGIN_URL); ?>includes/images/addon-images/main-logo.png" />
                                    ...
                                    <div class="ms-addon-installer-right-key-section" >
                                        ...
                                        if(isset($_COOKIE['ms_addon_return_data'])){
                                            $MJTC_addon_return_data = json_decode(MJTC_majesticsupportphplib::MJTC_safe_decoding(majesticsupport::MJTC_sanitizeData($_COOKIE['ms_addon_return_data'])),true);// MJTC_sanitizeData() function uses wordpress santize functions
                                            $MJTC_error_msg = $MJTC_addon_return_data;
                                            if(isset($MJTC_addon_return_data['status']) && $MJTC_addon_return_data['status'] == 0){
                                                $MJTC_error_message = $MJTC_addon_return_data['message'];
                                                $MJTC_transactionkey = $MJTC_addon_return_data['transactionkey'];
                                            }
                                            unset($MJTC_addon_return_data);
                                        }
                                        ?>
                                        <div class="ms-addon-installer-right-key-field" >
                                             <?php if($MJTC_error_message != '' ){ ?>
                                                <div class="ms-addon-installer-right-key-field-message" > <?php echo wp_kses_post($MJTC_error_message);?></div>
                                            <?php } ?>
                                            <input type="text" name="transactionkey" id="transactionkey" class="ms_key_field" value="<?php echo esc_attr($MJTC_transactionkey);?>" placeholder="<?php echo esc_attr(__('XXXX-XXXX-XXXXX-XXXXX','majestic-support')); ?>"/>
