...
class MJTC_gdprController {
    ...
    function handleRequest() {
        $MJTC_layout = MJTC_request::MJTC_getLayout('mjslay', null, 'gdpr');
        ...
        if (self::canaddfile($MJTC_layout)) {
            switch ($MJTC_layout) {
                ...
                case 'admin_addgdprfield':
                    ...
                    MJTC_includer::MJTC_getModel('fieldordering')->getUserFieldbyId($MJTC_id,3);
                    break;
            ...
            $MJTC_module = MJTC_request::MJTC_getVar($MJTC_module, null, 'gdpr');
    ...
    static function savegdprfield() {
        ...
        if (! wp_verify_nonce( $MJTC_nonce, 'save-gdprfield-'.$MJTC_id) ) {
    ...
    static function saveusereraserequest() {
        $MJTC_id = MJTC_request::MJTC_getVar('id');
        $MJTC_nonce = MJTC_request::MJTC_getVar('_wpnonce');
        if (! wp_verify_nonce( $MJTC_nonce, 'save-usereraserequest-'.$MJTC_id) ) {
            die( 'Security check Failed' );
        }
        $MJTC_data = MJTC_request::get('post');
    ...
    static function deletegdpr() {
        $MJTC_id = MJTC_request::MJTC_getVar('gdprid');
        $MJTC_nonce = MJTC_request::MJTC_getVar('_wpnonce');
        if (! wp_verify_nonce( $MJTC_nonce, 'delete-gdpr-'.$MJTC_id) ) {
            die( 'Security check Failed' );
        }
    ...
    static function removeusereraserequest() {
        $MJTC_id = MJTC_request::MJTC_getVar('majesticsupportid');
        $MJTC_nonce = MJTC_request::MJTC_getVar('_wpnonce');
        if (! wp_verify_nonce( $MJTC_nonce, 'delete-usereraserequest-'.$MJTC_id) ) {
            die( 'Security check Failed' );
        }
        MJTC_includer::MJTC_getModel('gdpr')->deleteUserEraseRequest($MJTC_id);
        $MJTC_url = majesticsupport::makeUrl(array('mjsmod'=>'gdpr', 'mjslay'=>'adderasedatarequest'));
        wp_safe_redirect($MJTC_url);
        exit;
    }
    ...
    static function exportusereraserequest() {
        ...
        if (! wp_verify_nonce( $MJTC_nonce, 'export-usereraserequest-'.$MJTC_uid) ) {
            die( 'Security check Failed' );
        }
        $MJTC_return_value = MJTC_includer::MJTC_getModel('gdpr')->setUserExportByuid($MJTC_uid);
        if (!empty($MJTC_return_value)) {
            // Push the report now!
            $msg = esc_html(__('User Data', 'majestic-support'));
            $MJTC_name = 'export-overalll-reports';
            header("Content-type: application/octet-stream");
            header("Content-Disposition: attachment; filename=" . esc_attr($MJTC_name) . ".xls");
            header("Pragma: no-cache");
            header("Expires: 0");
            header("Lacation: excel.htm?id=yes");
            print wp_kses($MJTC_return_value, MJTC_ALLOWED_TAGS);
            exit;
        }
    ...
    static function deleteuserdata() {
        $MJTC_uid  = MJTC_request::MJTC_getVar('majesticsupportid');
        $MJTC_nonce = MJTC_request::MJTC_getVar('_wpnonce');

        if (! wp_verify_nonce( $MJTC_nonce, 'delete-userdata-'.$MJTC_uid) ) {
            die( 'Security check Failed' );
        }
        $MJTC_return_value = MJTC_includer::MJTC_getModel('gdpr')->deleteUserData($MJTC_uid);
        $MJTC_url = admin_url("admin.php?page=majesticsupport_gdpr&mjslay=erasedatarequests");
        wp_safe_redirect($MJTC_url);
        die();
    }
    ...
    static function eraseidentifyinguserdata() {
        $MJTC_uid  = MJTC_request::MJTC_getVar('majesticsupportid');
        $MJTC_nonce = MJTC_request::MJTC_getVar('_wpnonce');
        if (! wp_verify_nonce( $MJTC_nonce, 'erase-userdata-'.$MJTC_uid) ) {
            die( 'Security check Failed' );
        }
        $MJTC_return_value = MJTC_includer::MJTC_getModel('gdpr')->anonymizeUserData($MJTC_uid);
        $MJTC_url = admin_url("admin.php?page=majesticsupport_gdpr&mjslay=erasedatarequests");
        wp_safe_redirect($MJTC_url);
        die();
    }
