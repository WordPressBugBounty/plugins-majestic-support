                ...
                <div class="post-installtion-content_wrapper_right">
                    <div class="mjtc-admin-title-installtion">
                        <span class="ms_heading">
                            ...
                            <?php
                                ...
                                $MJTC_steps = esc_html(__('Step 3 of ','majestic-support'));
                    ...
                    <form id="majesticsupport-form-ins" method="post" action="<?php echo esc_url(wp_nonce_url(admin_url("admin.php?page=majesticsupport_postinstallation&task=save&action=mstask"),"save")); ?>">
                        <div class="pic-config">
                            <div class="title">
                                <?php echo esc_html(__('Default alert email','majestic-support')); ?><?php echo esc_html(': ');?>
                            </div>
                            <div class="field">
                                <?php echo wp_kses(MJTC_formfield::MJTC_select('default_alert_email', majesticsupport::$_data[1] , isset(majesticsupport::$_data[0]['default_alert_email']) ? majesticsupport::$_data[0]['default_alert_email'] : '', '' , array('class' => 'inputbox ms-postsetting mjtc-select ms-postsetting ')), MJTC_ALLOWED_TAGS);?>
                            </div>
                            <div class="desc">
                                <?php echo esc_html(__('If the ticket department email is not selected, then this email is used to send emails','majestic-support')); ?>
                            </div>
                        </div>
                        <div class="pic-config">
                            <div class="title">
                                <?php echo esc_html(__('Default admin email','majestic-support')); ?><?php echo esc_html(': ');?>
                            </div>
                            <div class="field">
                                <?php echo wp_kses(MJTC_formfield::MJTC_select('default_admin_email', majesticsupport::$_data[1] , isset(majesticsupport::$_data[0]['default_admin_email']) ? majesticsupport::$_data[0]['default_admin_email'] : '', '' , array('class' => 'inputbox ms-postsetting mjtc-select ms-postsetting ')), MJTC_ALLOWED_TAGS);?>
                            </div>
                            <div class="desc">
                                <?php echo esc_html(__('Admin email address to receive emails','majestic-support')); ?>
                            </div>
                        </div>
