        ...
        if (is_user_logged_in()) { // wp user logged in
            ...
            if ($currentuser == '' and ($majesticsupport_registerform == '' && $registerform == 0)) {// inserting a record in our table
                ...
                if ($user_first && $user_last) {
                    ...
                    $display_name = sprintf(_x('%1$s %2$s', 'Display name based on first name and last name', 'majestic-support'), $user_first, $user_last);
                } elseif ($user_first) {
                ...
                $MJTC_data['id'] = '';
                $MJTC_data['wpuid'] = $wpuserid;
                $MJTC_data['name'] = $profile['name'];
                $MJTC_data['display_name'] = $display_name;
                $MJTC_data['user_nicename'] = $userdata->user_nicename;
                $MJTC_data['user_email'] = $userdata->user_email;
    ...
    function MJTC_deleteUserRecords($uid , $is_delete_user = false ){
        ...
        if(majesticsupport::$_db->query($query)){
            ...
            foreach ($MJTC_tickets as $MJTC_key) {
                $userpath = $path . '/' . $MJTC_key->ticketid;
                $files = glob($userpath.'/*');      
